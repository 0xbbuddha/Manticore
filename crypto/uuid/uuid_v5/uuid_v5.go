...
// UUIDv5 is a name-based UUID (version 5). It is derived from
// SHA-1(namespaceUUID || name) with version=5 and RFC 4122 variant.
type UUIDv5 struct {
	uuid.UUID
...
func (u *UUIDv5) Marshal() ([]byte, error) {
	...
	if u.Namespace == nil {
		return nil, fmt.Errorf("uuid_v5: missing namespace")
	}
	...
	// Contents of UUIDv5 is SHA-1(namespace || name)
	sha := sha1.New()
	sha.Write(nsBytes)
	...
	// Version fixed to 5, variant set to RFC 4122 (10xx), preserving lower two high-nibble bits
	u.UUID.Version = 5
...
func (u *UUIDv5) Unmarshal(marshalledData []byte) (int, error) {
	...
	// Check if this is a version 5 UUID
	if u.UUID.Version != 5 {
		return 0, fmt.Errorf("invalid UUID version: got %d, want 5", u.UUID.Version)
	}
...
// and returns a UUIDv5 structure.
//
// Parameters:
