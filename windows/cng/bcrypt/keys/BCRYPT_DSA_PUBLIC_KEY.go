...
//
// Parameters:
// - value: A byte slice containing the raw DSA public key to be parsed.
//
// Returns:
...
func (k *BCRYPT_DSA_PUBLIC_KEY) Unmarshal(value []byte) (int, error) {
	if len(value) < 24 {
		return 0, errors.New("buffer too small for BCRYPT_DSA_PUBLIC_KEY, header too short (at least 24 bytes are required)")
	}
	...
	if k.Magic.Magic != magic.BCRYPT_DSA_PUBLIC_MAGIC {
		return 0, fmt.Errorf("invalid DSA public key magic: 0x%08x", k.Magic.Magic)
	}
...
//
// Returns:
// - A byte slice representing the raw bytes of the BCRYPT_DSA_PUBLIC_KEY structure.
func (k *BCRYPT_DSA_PUBLIC_KEY) Marshal() ([]byte, error) {
	marshalledData := []byte{}

	// Marshalling magic
	k.Magic.Magic = magic.BCRYPT_DSA_PUBLIC_MAGIC
...
//
// Note:
// The function prints the Header and Data of the BCRYPT_DSA_PUBLIC_KEY structure.
...
func (k *BCRYPT_DSA_PUBLIC_KEY) Describe(indent int) {
	...
	fmt.Printf("%s<\x1b[93mBCRYPT_DSA_PUBLIC_KEY\x1b[0m>\n", indentPrompt)
