...
// It extracts the key size from the byte slice and stores it in the BCRYPT_ECC_KEY_BLOB structure.
func (b *BCRYPT_ECC_KEY_BLOB) Unmarshal(value []byte) (int, error) {
	if len(value) < 4 {
		return 0, errors.New("buffer too small for BCRYPT_ECC_KEY_BLOB, header too short (at least 4 bytes are required)")
	}
...
//
// Returns:
// - A byte slice representing the raw bytes of the BCRYPT_ECC_KEY_BLOB structure.
...
func (b *BCRYPT_ECC_KEY_BLOB) Describe(indent int) {
	...
	fmt.Printf("%s<\x1b[93mBCRYPT_ECC_KEY_BLOB (header)\x1b[0m>\n", indentPrompt)
