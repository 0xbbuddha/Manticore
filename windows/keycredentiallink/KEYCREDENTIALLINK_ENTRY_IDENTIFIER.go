...
*/
type KEYCREDENTIALLINK_ENTRY_IDENTIFIER struct {
	Value uint8
}
...
const (
	...
	KEYCREDENTIALLINK_ENTRY_IDENTIFIER_KeyID uint8 = 0x01

	// A SHA256 hash of all entries following this entry.
	KEYCREDENTIALLINK_ENTRY_IDENTIFIER_KeyHash uint8 = 0x02

	// Key material of the credential.
	KEYCREDENTIALLINK_ENTRY_IDENTIFIER_KeyMaterial uint8 = 0x03

	// Key Usage
	KEYCREDENTIALLINK_ENTRY_IDENTIFIER_KeyUsage uint8 = 0x04

	// Key Source
	KEYCREDENTIALLINK_ENTRY_IDENTIFIER_KeySource uint8 = 0x05

	// Device Identifier
	KEYCREDENTIALLINK_ENTRY_IDENTIFIER_DeviceId uint8 = 0x06

	// Custom key information.
	KEYCREDENTIALLINK_ENTRY_IDENTIFIER_CustomKeyInformation uint8 = 0x07

	// The approximate time this key was last used, in FILETIME format.
	KEYCREDENTIALLINK_ENTRY_IDENTIFIER_KeyApproximateLastLogonTimeStamp uint8 = 0x08

	// The approximate time this key was created, in FILETIME format.
	KEYCREDENTIALLINK_ENTRY_IDENTIFIER_KeyCreationTime uint8 = 0x09
)
...
func (k *KEYCREDENTIALLINK_ENTRY_IDENTIFIER) String() string {
	...
	default:
		return fmt.Sprintf("Unknown KEYCREDENTIALLINK_ENTRY_IDENTIFIER: %d", k.Value)
	}
}
